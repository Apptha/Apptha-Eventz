...
class paypal_class_events {
   ...
   var $fields = array();           // array holds the fields to submit to paypal
     ...
     function paypal_class_events() {

      // initialization constructor.  Called when class is created.
        $db = JFactory::getDBO();
        
        $query = " SELECT `mode` from `#__em_paypal_settings` ";
        $db->setQuery($query);
   ...
   function validate_ipn() {
      ...
      if(!$fp) {
         ...
         return false;
           ...
           $fsockpath=$url_parsed['path'];
           $fsockHost=$url_parsed['host'];
         // Post the data back to paypal
         fputs($fp, "POST $fsockpath HTTP/1.1\r\n");
         fputs($fp, "Host: $fsockHost\r\n");
